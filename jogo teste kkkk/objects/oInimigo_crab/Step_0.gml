...
if (knockback_duration > 0) {
    ...
    if (!place_meeting(new_x, new_y, Oparede)) {
        x = new_x; // Atualize a posição somente se não houver colisão
        y = new_y;
    } else {
        // Se houver colisão, você pode optar por não mover ou apenas mover um pouco
        // Tente mover o inimigo na direção do knockback até encontrar um espaço livre
